...
const normalizeData = (data: IPost[]): INormalizedData => {
  let normalizePosts: INormalizedData = {byId: {}, allIds: []}

  data.forEach((post: IPost): void => {
    normalizePosts.byId[post.id] = post;
    normalizePosts.allIds.push(post.id);
  });

  return normalizePosts;
};

export const taskResult2 = ():void => {
  console.log('task3: ', normalizeData(posts));
}
